...
# Behavior:
#   - If MEGABRAIN_AUTO_ROUTE not "true" → no-op
...
# Coexistence with risk-classification-enforcement.cjs (AC5):
#   - this hook runs on UserPromptSubmit; risk-classification on PreToolUse
#   - injection text avoids risk-trigger patterns
...
# Read user prompt from env (Claude Code may provide as CLAUDE_USER_PROMPT)
...
# Compose suggestion (avoid risk-classification patterns — keep simple)
...
if [[ "$WARN_ONLY" == "true" ]]; then
  ...
  # When promoted to BLOCK mode, would emit additionalContext via JSON
  cat <<JSON
